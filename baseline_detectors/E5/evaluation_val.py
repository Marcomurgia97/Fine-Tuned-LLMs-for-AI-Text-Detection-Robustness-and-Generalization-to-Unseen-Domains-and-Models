...
from evaluate_helpers import get_predicted_labels  
...
def calculate_metrics(
    ...
    (Mirrors evaluation.py structure, simplified: removed greedy/sampling specifics)
...
def evaluate_model(
	...
	model = AutoModelForSequenceClassification.from_pretrained("MayZhou/e5-small-lora-ai-generated-detector").to('cuda')    
	model_name_slug = 'e5'
    ...
    for example in tqdm(benchmark_dataset, desc="Processing validation set"):
        ...
        prediction = inference_model(model, [text_to_detect], nthreads=1)
    	print("Predicted Probability (machine-generated):", prediction.predictions['Predicted_Probs(1)'])

    	predicted_label = get_predicted_labels(prediction, 0.50)
    	print("Predicted Label:", predicted_label)
        # Store probability and true label
        scores.append(prediction.predictions['Predicted_Probs(1)'][0])
        labels.append(0 if is_human_truth else 1)

        is_machine_prediction = (predicted_label[0] == 1)
    ...
    # Calculate metrics based on the collected TP, FP, TN, FN (implicit 0.5 threshold)
