...
class AbstractLanguageChecker:
    ...
    def postprocess(self, token):
        ...
        :return: str -- cleaned_old and re-encoded token text
        """
        raise NotImplementedError
...
class LM(AbstractLanguageChecker):
    ...
    def check_probabilities(self, in_text, topk=40):
        ...
        # Truncate token_ids if its length is greater than 1024
        ...
        pred_topk = [
            list(
                zip(self.enc.convert_ids_to_tokens(topk_prob_inds[i]), topk_prob_values[i].data.cpu().numpy().tolist())
            )
    ...
    def sample_unconditional(self, length=100, topk=5, temperature=1.0):
        ...
        """
        context = torch.full((1, 1), self.enc.encoder[self.start_token], device=self.device, dtype=torch.long)
        prev = context
        output = context
        past = None
        # Forward through the model
    ...
    def check_probabilities(self, in_text, topk=40, max_context=20, batch_size=20):
        ...
        with torch.no_grad():
            for src, tgt in zip(input_batches, target_batches):
                ...
                logits = self.model(src, torch.zeros_like(src))[0][:, max_context + 1]
                ...
                # [(pos, prob), ...]
                real_topk_pos = list(
                    [int(np.where(sorted_preds[i] == tgt[i].item())[0][0]) for i in range(yhat.shape[0])]
                )
                ...
                pred_topk.extend(
                    [
                        list(
                            zip(
                                self.tokenizer.convert_ids_to_tokens(sorted_preds[i][:topk]),
